...
void CallGraphPass::doMLTA(Function *F) {
  ...
  std::string strBCPath = Ctx->ModuleMaps[F->getParent()];
  Ctx->BCFuncs[strBCPath][F->getName().str()] = F;
  ...
  for (inst_iterator i = inst_begin(F), e = inst_end(F);
		...
		if (CallInst *CI = dyn_cast<CallInst>(&*i)) {
			...
			FuncSet *FS = &Ctx->Callees[CI->getFunction()];
        ...
        FuncSet _FS;
        findCalleesWithMLTA(CI, _FS);
        FS->insert(_FS.begin(), _FS.end());   // gwangmu: #1

        auto modCaller = CI->getModule();

        // gwangmu
        // TODO: remove paths when deleted?
        Ctx->CandCallerPaths[callHash(CI)].insert(Ctx->ModuleMaps[modCaller]);

				for (Function *Callee : *FS) 
          Ctx->Callers[Callee].insert(CI->getFunction());
			...
			// Direct call
				...
				if (CF) {
					...
					FS->insert(CF);
          Ctx->Callers[CF].insert(CI->getFunction());

          Ctx->CallerPaths[Ctx->ModuleMaps[CF->getParent()]].insert(
              Ctx->ModuleMaps[CI->getModule()]);
				}
				// InlineAsm
				else {
...
bool CallGraphPass::doInitialization(Module *M) {
	...
	*OP << "#" << MIdx << " Initializing: " << M->getName().str() << "\n";
...
bool CallGraphPass::doFinalization(ModuleList &modules) {
  ...
  *OP<<"Mapping declaration functions to actual ones...\n";
  ...
  //for (auto CI : CallSet) {   // gwangmu: #1
  //for (auto &_p1 : Ctx->BCFuncs) {
  //  for (auto &_p2 : _p1.second) {
  //    Function *CF = _p2.second;
  for (auto &_p1 : modules) {
    for (auto &_F : _p1.first->functions()) {
      Function *CF = &_F;
			FuncSet FS;
      for (auto F: Ctx->Callees[CF]) {
				if (F->isDeclaration()) {
...
// gwangmu: #1: changing to PlainFuncMap.
