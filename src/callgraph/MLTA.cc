		...
		for (auto oi = U->op_begin(), oe = U->op_end(); 
          ...
          /*
            ...
            */
            ...
            funcTypeIdxsMap[FoundF].insert(hashidx_c(TyH, 0));
#else
						typeIdxFuncsMap[TyH][Container.second].insert(FoundF);
            funcTypeIdxsMap[FoundF].insert(hashidx_c(TyH, Container.second));
#endif
						//DBG<<"[HASH] "<<TyH<<"\n";
		...
		if (PO && VO) {
			...
			if (!TyList.empty()) {
				for (auto TyIdx : TyList) {
					propagateType(F->getParent(), PO, TyIdx.first, TyIdx.second);
				}
				continue;
			}
			...
			// Composite type
			if (BTy) {
				propagateType(F->getParent(), PO, BTy);
				continue;
			}
			...
			if (FTy) {
				if (!getBaseFunction(VO)) {
					propagateType(F->getParent(), PO, FTy);
					continue;
				}
				else
					continue;
...
void MLTA::confineTargetFunction(Value *V, Function *F) {
    ...
    size_t _hash = typeHash(TI.first);
		typeIdxFuncsMap[_hash][TI.second].insert(F);
    funcTypeIdxsMap[F].insert(hashidx_c(_hash, TI.second));
	}
	if (!Complete) {
		if (!TyChain.empty())
...
void MLTA::propagateType(Module *M, Value *ToV, Type *FromTy, int Idx) {
    ...
    size_t _hash = typeHash(T.first);
    hashidx_t _prophash = hashidx_c(typeHash(FromTy), Idx);
		typeIdxPropMap[_hash][T.second].insert(_prophash);
    /*
    if (modTypeIdxPropMap[M][_hash][T.second].find(_prophash) == 
        modTypeIdxPropMap[M][_hash][T.second].end()) {
      typeIdxPropCntMap[_hash][T.second][_prophash]++;
      modTypeIdxPropMap[M][_hash][T.second].insert(_prophash);
    }
    */
		//DBG<<"[PROP] "<<*(FromTy)<<": "<<Idx
		//	<<"\n\t===> "<<*(T.first)<<" "<<T.second<<"\n";
...
void MLTA::printTargets(FuncSet &FS, CallInst *CI) {
	...
	*OP<<"\n\t Indirect-call targets: ("<<FS.size()<<")\n";
	for (auto F : FS) {
		if (F->isDeclaration()) {
			*OP<<"ERROR: print declaration function: "<<F->getName().str()<<"\n";
			continue;
		}
...
// gwangmu
void MLTA::initTempStates() {
  MatchedFuncsMap.clear();
  AliasStructPtrMap.clear();
  Int8PtrTy.clear();
  IntPtrTy.clear();
  DLMap.clear();
}

void MLTA::invalidateStates(Module *M) {
  for (auto &_F : M->functions()) {
    Function *F = &_F;
    if (funcTypeIdxsMap.find(F) == funcTypeIdxsMap.end())
      continue;

    set<hashidx_t> &typeIdxs = funcTypeIdxsMap[F];
    for (auto _h : typeIdxs) {
      typeIdxFuncsMap[_h.first][_h.second].erase(F);
      if (typeIdxFuncsMap[_h.first][_h.second].empty()) {
        typeIdxFuncsMap[_h.first].erase(_h.second);
        if (typeIdxFuncsMap[_h.first].empty()) 
          typeIdxFuncsMap.erase(_h.first);
      }
    }

    funcTypeIdxsMap.erase(F);
  }

  /*
  auto &_modTypeIdxPropMap = modTypeIdxPropMap[M];
  for (auto &_p1 : _modTypeIdxPropMap) {
    size_t _hash = _p1.first;
    for (auto &_p2 : _p1.second) {
      int _idx = _p2.first;
      for (auto &_prophash : _p2.second) {
        typeIdxPropCntMap[_hash][_idx][_prophash]--;
        assert(typeIdxPropCntMap[_hash][_idx][_prophash] >= 0);
        if (typeIdxPropCntMap[_hash][_idx][_prophash] == 0) 
          typeIdxPropMap[_hash][_idx].erase(_prophash);
      }
    }
  }
  */
